...
classfile read_classfile(const char* filename)
{
  ...
  for (int method_counter = 0; method_counter < classfile.method_count; method_counter++) {
    ...
    printf("\t\tmethod:\t%x\n", classfile.methods[method_counter]);
  }
  printf("\t(+) [METHOD INFO END]\n");

  fread(&classfile.attribute_count, sizeof(classfile.attribute_count), 1, fileptr);
  classfile.attribute_count = be16toh(classfile.attribute_count);
  printf("\n\tATTRIBUTE COUNT:\t%d\n", classfile.attribute_count);

  classfile.attributes = malloc((sizeof(attribute_info) * classfile.attribute_count));
  printf("\n\t(-) ATTRIBUTE INFO START\n");
  for(int att_iterator = 0; att_iterator < classfile.attribute_count; att_iterator++){
    classfile.attributes[att_iterator] = parse_attribute_info(fileptr);
    printf("\t\t%d: attribute:\t%d\n", att_iterator, classfile.attributes[att_iterator]);
  }
